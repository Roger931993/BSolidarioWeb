...
//   userList,
// } from '../data';
import { cartData } from '@web/core/data/cart';
      ...
      of(null)
        .pipe(
          mergeMap(() => {
            ...
            // get recentFiles
            if (
              request.url.endsWith('/app/recentFiles') &&
              request.method === 'GET'
            ) {
              if (recentFiles) {
            ...
            // get Orderdata
            if (
              request.url.endsWith('/app/orderData') &&
              request.method === 'GET'
            ) {
              if (ordersData) {
            ...
            // get projects
            if (
              request.url.endsWith('/app/projects') &&
              request.method === 'GET'
            ) {
              if (projectData) {
            ...
            // get userGridData
            if (
              request.url.endsWith('/app/userGrid') &&
              request.method === 'GET'
            ) {
              if (userGridData) {
                return of(
                  new HttpResponse({ status: 200, body: userGridData })
                );
              } else {
                return throwError({
                  status: 401,
            ...
            // get userGridData
            if (
              request.url.endsWith('/app/userlist') &&
              request.method === 'GET'
            ) {
              if (userList) {
            ...
            // get joblist
            if (
              request.url.endsWith('/app/joblist') &&
              request.method === 'GET'
            ) {
              if (JobListdata) {
            ...
            // get joblist
            if (
              request.url.endsWith('/app/jobgrid') &&
              request.method === 'GET'
            ) {
              if (JobGriddata) {
            ...
            // get joblist
            if (
              request.url.endsWith('/app/jobapply') &&
              request.method === 'GET'
            ) {
              if (JobApplydata) {
                return of(
                  new HttpResponse({ status: 200, body: JobApplydata })
                );
              } else {
                return throwError({
                  status: 401,
            ...
            // get candidates
            if (
              request.url.endsWith('/app/candidate') &&
              request.method === 'GET'
            ) {
              if (CandidateList) {
                return of(
                  new HttpResponse({ status: 200, body: CandidateList })
                );
              } else {
                return throwError({
                  status: 401,
            ...
            // get invoice
            if (
              request.url.endsWith('/app/invoice') &&
              request.method === 'GET'
            ) {
              if (listData) {
            ...
            // get chatmessage
            if (
              request.url.endsWith('/app/chatMessage') &&
              request.method === 'GET'
            ) {
              if (chatMessagesData) {
                return of(
                  new HttpResponse({ status: 200, body: chatMessagesData })
                );
              } else {
                return throwError({
                  status: 401,
            ...
            // get Tasks
            if (
              request.url.endsWith('/app/tasks') &&
              request.method === 'GET'
            ) {
              if (tasks) {
            ...
            // get chatmessage
            if (
              request.url.endsWith('/app/cryto') &&
              request.method === 'GET'
            ) {
              if (orders) {
            ...
            // get userGridData
            if (
              request.url.endsWith('/app/customersData') &&
              request.method === 'GET'
            ) {
              if (customersData) {
                return of(
                  new HttpResponse({ status: 200, body: customersData })
                );
              } else {
                return throwError({
                  status: 401,
            ...
            // get chatmessage
            if (
              request.url.endsWith('/app/emailData') &&
              request.method === 'GET'
            ) {
              if (emailData) {
